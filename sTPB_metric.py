...
class query_TPB_Metric(CWLMetric):
    def __init__(self, br, G=1.0, C=1.0, gamma=1 ):
        super().__init__()
    ...
    def c_examine_vector(self, ranking, worse_case=True):
        ...
        for i in range(len(c_gains)):
            ...
            indicator = rate - self.gamma
            
            if indicator >= 0:
                coefficient = 1 - (sigmoid(indicator, self.b3, -self.R3) + sigmoid(indicator, self.b3, self.R3)) / 2
            else:
                coefficient = 1 - sigmoid(indicator, self.b3, self.R3)
    ...
    def return_G_C_i(self, ranking, stop_rank, worse_case=True):
        ...
        if G[-1] > 0.5:
            ...
            return 0.5, C[stop_rank - 1]

class sTPB_Metric(query_TPB_Metric):
    def __init__(self, G_0, C_0, gamma, br, M, N=10):
        super().__init__(br)
    ...
    def F_and_C_vector(self, session):
        ...
        for query_id in range(self.M):
            ...
            metric_imp = query_TPB_Metric(G=G_t, C=C_t, gamma=self.gamma, br=self.bounded_rationality)
